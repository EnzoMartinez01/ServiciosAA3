...
public class AlumnoCursoServiceImpl implements AlumnoCursoService {
	...
	public ResponseDto getAllAlumnoCurso() {
		//Para convertir un json a un obejto java
		...
		try {
			...
			log.error("RetryableException: " + Constante.SERVICE_NOT_AVAILABLE + " " + ex);
			return Util.getResponse(false, Constante.SERVICE_NOT_AVAILABLE, null);
		} catch (Exception e) {
	...
	public ResponseDto getAlumnoCurso(Long id) {
		...
		try {
			...
			if(alumnoCursoEntity == null) {
				return Util.getResponse(true, Constante.NO_RECORDS_FOUND, alumnoCursoEntity);
			}
